...
fig = go.Figure(data=[go.Ohlc(x=df['Date'], open=df['HA_Open'],high=df['HA_High'], low=df['HA_Low'], close=df['HA_Close'])])
fig.show()

if (float(df.iloc[[-2]]['HA_Close']) <= float(df.iloc[[-2]]['HA_Open'])):
	if (float(df.iloc[[-1]]['HA_Close']) >= float(df.iloc[[-1]]['HA_Open'])):
		if(float(df.iloc[[-1]]['HA_High'])>float(df.iloc[[-2]]['HA_High']) and float(df.iloc[[-1]]['HA_Low'])<float(df.iloc[[-2]]['HA_Low'])):
