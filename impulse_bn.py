...
df = df['LTP'].resample('30min', offset="15m").ohlc().dropna()
...
df2 = df2['LTP'].resample('30min', offset="15m").ohlc().dropna()
# df=df.drop_duplicates(inplace=False)
print((df2))

if (float(df.iloc[[2]]['close']) > float(df.iloc[[2]]['open']) and float(df2.iloc[[2]]['close']) < float(
        df2.iloc[[2]]['open'])):
...
elif (float(df.iloc[[2]]['close']) < float(df.iloc[[2]]['open']) and float(df2.iloc[[2]]['close']) > float(
        df2.iloc[[2]]['open'])):
